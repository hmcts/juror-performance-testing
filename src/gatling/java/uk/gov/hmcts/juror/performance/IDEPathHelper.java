...
public class IDEPathHelper {
  ...
  static {
    try {
      Path projectRootDir =
          Paths.get(requireNonNull(IDEPathHelper.class.getClassLoader().getResource("gatling.conf"), "Couldn't "
          + "locate gatling.conf").toURI()).getParent().getParent().getParent().getParent();
